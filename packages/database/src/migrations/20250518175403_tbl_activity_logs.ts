...
export async function up(knex: Knex): Promise<void> {
    await knex.schema.createTable('activity_logs', table => {
        ...
        table.enum('device_type', ['1', '2', '3']).defaultTo('2').notNullable().comment('1 -> Mobile, 2 -> Desktop, 3 -> Tablet');
        table.enum('activity_type', ['1', '2']).notNullable().comment('1 -> Login, 2 -> Logout');
