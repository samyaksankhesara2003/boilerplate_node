...
const getExperience = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  try {
    ...
    return sendResponse(res, StatusCodes.SUCCESS, ResponseMessages.EXPERIENCE.FETCH_SUCCESS, data, language);
  } catch (error) {
    next(error);
  }
};
...
const listExperience = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  try {
    ...
    return sendResponse(res, StatusCodes.SUCCESS, ResponseMessages.EXPERIENCE.LIST_SUCCESS, data, language);
  } catch (error) {
    next(error);
  }
};
